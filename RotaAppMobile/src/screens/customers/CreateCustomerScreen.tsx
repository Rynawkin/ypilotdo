...
const CreateCustomerScreen: React.FC = () => {
  ...
  // Sıradaki müşteri kodunu bul
  const getNextCustomerCode = async (): Promise<string> => {
    try {
      // Tüm müşterileri çek
      const customers = await customerService.getAll();
      
      // MUS formatındaki kodları filtrele
      const existingCodes = customers
        .map(c => c.code)
        .filter(code => code && code.match(/^MUS\d{3}$/))
        .map(code => parseInt(code.replace('MUS', '')))
        .sort((a, b) => a - b);
      
      // Sıradaki boş kodu bul
      let nextNumber = 1;
      ...
      return `MUS${nextNumber.toString().padStart(3, '0')}`;
    } catch (error: any) {
    ...
    try {
      ...
      if (isOnline) {
        ...
        await offlineQueueService.addToQueue({
          ...
          data: submitData,
          journeyId: 0, // Customer create için gerekli değil
        });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <Button
              ...
              onPress={async () => {
                try {
                  ...
                  const errorMessage = error.userFriendlyMessage || error.message || 'Sıradaki kod alınamadı';
                  Alert.alert('Hata', errorMessage);
                }
              }}
              style={styles.nextCodeButton}
              contentStyle={styles.nextCodeButtonContent}
              labelStyle={styles.nextCodeButtonLabel}
            >
              Sıradaki Kodu Getir
            </Button>
          </View>
            ...
            <Switch
              ...
              onValueChange={(value) => {
                setFormData({
                  ...formData,
                  whatsAppOptIn: value,
                  whatsAppOptInDate: value ? new Date().toISOString() : undefined
                });
              }}
              color="#25D366"
            />
          ...
          <Card.Content>
            <View style={styles.offlineIndicator}>
              ...
              <Text style={styles.offlineText}>
                Çevrimdışı - Müşteri taslak olarak kaydedilecek
              </Text>
            </View>
          </Card.Content>
        </Card>
      ...
      <View style={styles.submitContainer}>
        ...
        >
          {loading ? 'Kaydediliyor...' : isOnline ? 'Müşteri Oluştur' : 'Taslak Kaydet'}
        </Button>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  codeInput: {
    flex: 1,
    marginBottom: 0, // TextInput'un alt marginini kaldır
  },
  nextCodeButton: {
    backgroundColor: '#10B981',
    borderRadius: 6,
    height: 36, // Daha kısa buton
    minWidth: wp(28), // Minimum genişlik
  },
  nextCodeButtonContent: {
    height: 36,
